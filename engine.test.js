...
// Regression: a real IEEE manuscript whose reference list uses WORD'S OWN auto-numbered-list
// style (the "1.", "2.", ... prefix is generated by numbering.xml, not literal text) reads,
// once extracted from the .docx, with NO leading number on any reference line at all — the
// exact same gap parseReferenceLine already works around for the numeric family (see its
// "references.every(numLabel == null) -> assign by position" fallback). Before this fix,
// FormatDetector had no matching safeguard: with the "[1]"/"1. " prefix signals silently
// absent, IEEE's other cues (vol./no./pp., a quoted title) were not distinctive enough against
// MLA9's near-identical "vol./no./pp." cues, and MLA9 could outscore IEEE on a coin-flip
// margin — even though EVERY reference opens "F. Last," (initials-then-surname), a shape
// MLA9/APA7/Harvard/Chicago never produce (they always invert the first author to "Last, F.").
test('IEEE reference list with Word auto-numbering (no literal "1."/"[1]" prefix) still detected as ieee, not mla9', () => {
  const article = 'Prior work has studied this extensively [1], [2]. Others agree [3], [4], [5].';
  const references = [
    'H. Crompton and D. Burke, "Artificial intelligence in higher education: the state of the field," Int. J. Educ. Technol. High. Educ., vol. 20, no. 1, art. 22, 2023, doi: 10.1186/s41239-023-00392-8.',
    'O. Zawacki-Richter, V. I. Marín, M. Bond, and F. Gouverneur, "Systematic review of research on artificial intelligence applications in higher education," Int. J. Educ. Technol. High. Educ., vol. 16, no. 1, art. 39, 2019, doi: 10.1186/s41239-019-0171-0.',
    'R. Parasuraman, T. B. Sheridan, and C. D. Wickens, "A model for types and levels of human interaction with automation," IEEE Trans. Syst., Man, Cybern. A, Syst., Humans, vol. 30, no. 3, pp. 286-297, 2000, doi: 10.1109/3468.844354.',
    ...
    'C. K. Lo, "What is the impact of ChatGPT on education? A rapid review of the literature," Educ. Sci., vol. 13, no. 4, art. 410, 2023, doi: 10.3390/educsci13040410.',
  ].join('\n'); // deliberately no "1. "/"[1] " prefix — mimics Word auto-numbering being lost on plain-text extraction
  const d = CE.FormatDetector.detect(article, references);
  assert.strictEqual(d.styleId, 'ieee', 'expected ieee, got ' + d.styleId + ' — scores: ' + JSON.stringify(d.scores));
  assert.ok(d.scores.ieee > d.scores.mla9, 'ieee should clearly beat mla9, got ' + JSON.stringify(d.scores));
});
